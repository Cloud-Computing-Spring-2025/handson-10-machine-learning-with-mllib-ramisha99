...
def preprocess_data(df):
    ...
    encoders = [OneHotEncoder(inputCol=c + "Index", outputCol=c + "Vec") for c in categorical_cols]

    label_indexer = StringIndexer(inputCol="Churn", outputCol="ChurnIndex")
    numeric_cols = ['SeniorCitizen', 'tenure', 'MonthlyCharges', 'TotalCharges']

    assembler_inputs = [c + "Vec" for c in categorical_cols] + numeric_cols
    assembler = VectorAssembler(inputCols=assembler_inputs, outputCol="features")

    pipeline_stages = indexers + encoders + [label_indexer, assembler]
...
def feature_selection(df):
    ...
    result = selector.fit(df).transform(df).select("selectedFeatures", "ChurnIndex")
    result.show(5)
...
def tune_and_compare_models(df):
    ...
    print(f"Best Logistic Regression AUC: {round(auc_lr, 2)} | Params: regParam={best_lr._java_obj.getRegParam()}, maxIter={best_lr._java_obj.getMaxIter()}")
    ...
    print(f"Best Decision Tree AUC: {round(auc_dt, 2)} | Params: maxDepth={best_dt._java_obj.getMaxDepth()}")
    ...
    print(f"Best Random Forest AUC: {round(auc_rf, 2)} | Params: maxDepth={best_rf._java_obj.getMaxDepth()}, numTrees={best_rf._java_obj.getNumTrees()}")
    ...
    print(f"Best GBT AUC: {round(auc_gbt, 2)} | Params: maxDepth={best_gbt._java_obj.getMaxDepth()}, maxIter={best_gbt._java_obj.getMaxIter()}")
